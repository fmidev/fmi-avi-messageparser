...
import static fi.fmi.avi.tac.lexer.Lexeme.Identity;
...
import fi.fmi.avi.tac.lexer.Lexeme.ParsedValueName;
...
import fi.fmi.avi.tac.lexer.impl.token.RunwayState.RunwayStateContamination;
import fi.fmi.avi.tac.lexer.impl.token.RunwayState.RunwayStateDeposit;
import fi.fmi.avi.tac.lexer.impl.token.RunwayState.RunwayStateReportSpecialValue;
...
public class METARTACParser extends AbstractAviMessageParser implements TACParser<String, METAR> {
    ...
    public ConversionResult<METAR> convertMessage(final String input, final ConversionHints hints) {
        ConversionResult<METAR> result = new ConversionResultImpl<>();
        ...
        if (endsInEndToken(lexed, hints)) {
            ...
            findNext(CORRECTION, lexed.getFirstLexeme(), stopAt, (match) -> result.getConvertedMessage().setStatus(AviationCodeListUser.MetarStatus.CORRECTION),
            ...
            findNext(AUTOMATED, lexed.getFirstLexeme(), stopAt, (match) -> result.getConvertedMessage().setAutomatedStation(true));
            ...
            stopAt = new Identity[] { HORIZONTAL_VISIBILITY, RUNWAY_VISUAL_RANGE, CLOUD, AIR_DEWPOINT_TEMPERATURE, AIR_PRESSURE_QNH, RECENT_WEATHER, WIND_SHEAR,
                    SEA_STATE, RUNWAY_STATE, COLOR_CODE, FORECAST_CHANGE_INDICATOR, REMARKS_START };
            findNext(CAVOK, lexed.getFirstLexeme(), stopAt, (match) -> result.getConvertedMessage().setCeilingAndVisibilityOk(true));
    ...
    private static void updateClouds(final ConversionResult<METAR> result, final LexemeSequence lexed, final ConversionHints hints) {
        ...
        if (msg.getClouds() != null && msg.isNoSignificantCloud()) {
        	result.addIssue(new ConversionIssue(ConversionIssue.Type.SYNTAX_ERROR, "Message contains both clouds as well as NSC"));
        }
    }
    ...
    private static void updateSeaState(final ConversionResult<METAR> result, final LexemeSequence lexed, final ConversionHints hints) {
        ...
        findNext(SEA_STATE, lexed.getFirstLexeme(), before, (match) -> {
            ...
            if (values[1] instanceof fi.fmi.avi.tac.lexer.impl.token.SeaState.SeaSurfaceState) {
                if (values[2] != null) {
                    result.addIssue(new ConversionIssue(Type.LOGICAL_ERROR,
                            "Sea state cannot contain both sea surface state and significant wave height:" + match.getTACToken()));
                } else {
                    switch ((fi.fmi.avi.tac.lexer.impl.token.SeaState.SeaSurfaceState) values[1]) {
                        case CALM_GLASSY:
    ...
    private static void updateRunwayStates(final ConversionResult<METAR> result, final LexemeSequence lexed, final ConversionHints hints) {
        ...
        findNext(RUNWAY_STATE, lexed.getFirstLexeme(), before, (match) -> {
        	...
        	while (match != null){
	        	...
	        	if (deposit != null) {
	        		AviationCodeListUser.RunwayDeposit value = fi.fmi.avi.tac.lexer.impl.token.RunwayState.convertRunwayStateDepositToAPI(deposit);
	        		if (value != null) {
	        	...
	        	if (contamination != null) {
	        		AviationCodeListUser.RunwayContamination value = fi.fmi.avi.tac.lexer.impl.token.RunwayState.convertRunwayStateContaminationToAPI(contamination);
	        		if (value != null) {
				...
				if (breakingAction instanceof fi.fmi.avi.tac.lexer.impl.token.RunwayState.BreakingAction) {
					BreakingAction action = 
							fi.fmi.avi.tac.lexer.impl.token.RunwayState.convertBreakingActionToAPI(
									(fi.fmi.avi.tac.lexer.impl.token.RunwayState.BreakingAction)breakingAction);
                            ...
                            switch (type) {
                                case AT: {
                                    ...
                                    if (fromHour != null) {
                                        ...
                                        result.addIssue(new ConversionIssue(Type.SYNTAX_ERROR, "Missing hour from trend AT group " + token.getTACToken()));
                                    }
                                    if (fromMinute != null) {
                                        timeGroups.setFromMinute(fromMinute);
                                    } else {
                                        result.addIssue(new ConversionIssue(Type.SYNTAX_ERROR, "Missing minutes from trend AT group " + token.getTACToken()));
                                    }
                                ...
                                case FROM: {
                                    ...
                                    if (fromHour != null) {
                                        ...
                                        result.addIssue(new ConversionIssue(Type.SYNTAX_ERROR, "Missing hour from trend FM group " + token.getTACToken()));
                                    }
                                    if (fromMinute != null) {
                                        timeGroups.setFromMinute(fromMinute);
                                    } else {
                                        result.addIssue(new ConversionIssue(Type.SYNTAX_ERROR, "Missing minutes from trend FM group " + token.getTACToken()));
                                    }
                                    fct.setTimeGroups(timeGroups);
                                    break;
                                }
                                case UNTIL: {
                                    ...
                                    if (toHour != null) {
                                        ...
                                        result.addIssue(new ConversionIssue(Type.SYNTAX_ERROR, "Missing hour from trend TL group " + token.getTACToken()));
                                    }
                                    if (toMinute != null) {
                                        timeGroups.setToMinute(toMinute);
                                    } else {
                                        result.addIssue(new ConversionIssue(Type.SYNTAX_ERROR, "Missing minutes from trend TL group " + token.getTACToken()));
                                    }
                                    fct.setTimeGroups(timeGroups);
                                    break;
                                }
                    ...
                    while (token != null) {
                        switch (token.getIdentity()) {
                            ...
                            case WEATHER: {
                                ...
                                List<ConversionIssue> issues = new ArrayList<>(1);
